...
BDD_INST_CATEGORIES = [
    ...
    {"color": [255, 109, 65], "isthing": 1, "id": 2, "name": "rider"},
    {"color": [255, 77, 255], "isthing": 1, "id": 3, "name": "car"},
    {"color": [0, 226, 252], "isthing": 1, "id": 4, "name": "truck"},
    {"color": [182, 182, 255], "isthing": 1, "id": 5, "name": "bus"},
    {"color": [0, 125, 92], "isthing": 1, "id": 6, "name": "train"},
    {"color": [120, 166, 157], "isthing": 1, "id": 7, "name": "motorcycle"},
    {"color": [110, 76, 0], "isthing": 1, "id": 8, "name": "bicycle"}
]
...
def _get_bdd_obj_track_meta():
    thing_ids = [k["id"] for k in BDD_TRACK_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in BDD_TRACK_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 8, len(thing_ids)
    # Mapping from the incontiguous category id to a contiguous id in [0, C-1]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in BDD_TRACK_CATEGORIES if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        for frame_idx in range(record["length"]):
            ...
            for anno in anno_dict_list:
                ...
                #     print(_segm is None)
                #     print(_segm[frame_idx] is None)
                #     print(f"frame {frame_idx} has no mask.")
                #     continue
