...
class TemporalRefiner(nn.Module):
    def __init__(
        ...
        # self.padding_embed = nn.Embedding(1, hidden_channel)
        # self.slot_padding_embed = nn.Embedding(1, hidden_channel)
        # self.padding_embed = nn.Linear(hidden_channel, hidden_channel)
        self.padding_embed = nn.Identity()

    def forward(self, instance_embeds, padding_mask, frame_embeds, mask_features, matched_gt_ids):
        ...
        for i in range(self.num_layers):
            ...
            # tgt_key_padding_mask = padding_mask.clone().flatten(0, 1)  # bq, t
            # tgt_key_padding_mask[torch.sum(tgt_key_padding_mask, dim=1)==n_frames, :] = False
            ...
            # tgt_key_padding_mask2 = padding_mask.clone().permute(0, 2, 1).flatten(0, 1)  # b, q, t -> b, t, q -> bt, q
            # tgt_key_padding_mask2[torch.sum(tgt_key_padding_mask2, dim=1)==n_instance, :] = False
            output = self.transformer_obj_self_attention_layers[i](
                output, tgt_mask=None,
                # tgt_key_padding_mask=tgt_key_padding_mask2,
                query_pos=None
            )
        ...
        outputs_class, outputs_masks = self.prediction(outputs, mask_features, padding_mask)  # (l, b, q, t, k+1), (l, b, q, t, h, w)
        # outputs = self.decoder_norm(outputs)

        out = {
            'pred_logits': outputs_class[-1].transpose(1, 2),  # (b, t, q, c)
            'pred_masks': outputs_masks[-1],  # (b, q, t, h, w)
            'aux_outputs': self._dvis_set_aux_loss(
    ...
    @torch.jit.unused
    def _dvis_set_aux_loss(self, outputs_class, outputs_seg_masks):
    ...
    def windows_prediction(self, outputs, mask_features, padding_masks,  windows=5):
        """
        ...
        outputs_classes = self.pred_class(outputs_classes, padding_masks)
        return outputs_classes.cpu().to(torch.float32), torch.cat(outputs_masks, dim=3)

    def pred_class(self, decoder_output, padding_masks):
        """
    ...
    def prediction(self, outputs, mask_features, padding_mask):
        """
        ...
        if self.training:
            ...
            decoder_output = decoder_output.permute(1, 3, 0, 2, 4)  # (l, b, t, q, c)
            outputs_class = self.pred_class(decoder_output, padding_mask)
            mask_embed = self.mask_embed(decoder_output)
            outputs_mask = torch.einsum("lbtqc,btchw->lbqthw", mask_embed, mask_features)
        else:
            outputs = outputs[:, -1:]
            outputs_class, outputs_mask = self.windows_prediction(outputs, mask_features, padding_mask, windows=self.windows)
