...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        # patch_embed, convert image to patch embeddings
    ...
    def forward_features(self, x, masks=None):
        ...
        return {
            "x_norm_clstoken": x_norm[:, 0],
            "x_norm_patchtokens": x_norm[:, 1:],  # (B, HW, C)
            "x_prenorm": x,  # (B, 1 + HW, C)
            "masks": masks,
        }
...
def get_models(name='vitl', weight=None):
    ...
    model = model_dicts[name](**vit_kwargs)
    if weight is not None:
        load_pretrained_weights(model, weight, None)
    else:
        logger.warning("No Pretrained Weights !!!")
    return model
