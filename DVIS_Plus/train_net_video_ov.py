...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        assert len(cfg.DATASETS.DATASET_RATIO) == len(cfg.DATASETS.TRAIN) == len(cfg.DATASETS.DATASET_TYPE)
        mappers = []
        mapper_dict = {
        ...
        for d_i, (dataset_name, dataset_type) in \
                enumerate(zip(cfg.DATASETS.TRAIN, cfg.DATASETS.DATASET_TYPE)):
            if dataset_type not in mapper_dict.keys():
                raise NotImplementedError
            _mapper = mapper_dict[dataset_type]
            mappers.append(
                _mapper(cfg, is_train=True, is_tgt=True, src_dataset_name=dataset_name, )
            )
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="ov_DVIS")
    return cfg
