...
def register_all_video_panoVSPW_ov(root):
    for (
            prefix,
            (image_root, panoptic_root, panoptic_json),
    ) in _PREDEFINED_SPLITS_PANOVSPW.items():
        metadata = get_metadata_ov(os.path.join(root, panoptic_json))
...
register_all_video_panoVSPW_ov(_root)
